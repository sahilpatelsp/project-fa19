...
def solve(list_of_locations, list_of_homes, starting_car_location, adjacency_matrix, params=[]):
    ...
    try:
        ...
        m.addConstrs((e.sum('*', start_index) >= 1) for i in range(loc_len))
        m.addConstrs((e.sum(start_index, '*') >= 1) for i in range(loc_len))
        ...
        # m.addConstrs((v[i] == 0) >> (u[i] == 0) for i in range(loc_len) if i != start_index)
        ...
        for i in range(loc_len):
            if i != start_index:
                for j in range(loc_len):
                    if j != start_index:
                        m.addQConstr((u[i] - u[j] + 1) <= ((loc_len - 1) * (1 - e[i, j])))
        ...
        for var in m.getVars():
            if var.x >= 1 and "p" not in var.varName:
